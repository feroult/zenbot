...
module.exports = function container (conf) {
  ...
  function authedClient() {
    if (!authed_client) {
      if (!conf.mercado || !conf.mercado.key || !conf.mercado.key === 'YOUR-API-KEY') {
        throw new Error('please configure your mercado credentials in ' + path.resolve(__dirname, 'conf.js'))
      }
  ...
  var exchange = {
    ...
    getTrades: function (opts, cb) {
      ...
      {
        var params = {}
        if(opts.from < 99999999999) {
          params.tid = opts.from - 1
        } else {
          params.from = Math.floor(opts.from/1000) + ''
        }
